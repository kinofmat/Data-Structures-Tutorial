...
"""
def undo():
    #Remember to check that the stack is not empty.
    if len(doc) > 0:
        temp = doc.pop()
        letter_holder.append(temp)
        display_phrase(doc)
    else:
        print("There is nothing else to undo!")
...
def redo():
    #Remember to check that the stack is not empty.
    if len(letter_holder) > 0:
        temp = letter_holder.pop()
        doc.append(temp)
        display_phrase(doc)
    else:
        print("There is nothing else to redo!")
